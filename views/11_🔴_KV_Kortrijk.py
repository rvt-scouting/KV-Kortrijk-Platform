...
# -----------------------------------------------------------------------------
# 1. SETUP & DYNAMISCHE FILTERS (3 RETURN VALUES)
# -----------------------------------------------------------------------------
...
for db_pos, display_label in display_positions:
    ...
    query = f"""
        ...
        WHERE pfs."squadId"::text = %s AND pfs."iterationId"::text = %s AND pfs.position = %s
        AND pfs.metric_id IN ({ids_str})
    """
    df_pos = run_query(query, (squad_id, iteration_id, db_pos))

    if not df_pos.empty:
        # Individuele Tabel (Met pivot_table tegen duplicaten)
        ...
        if not weak_metrics.empty:
            ...
            with st.expander(f"🎯 Versterkingen voor: {', '.join(weak_names)}"):
                ...
                if not df_targets_raw.empty:
                    # GEBRUIK pivot_table om duplicates error te voorkomen
                    df_target_pivot = df_targets_raw.pivot_table(index=['Naam', 'Club'], columns='metric_name', values='score', aggfunc='mean')
                    ...
                    st.write("Geen spelers gevonden met score > 60 op deze punten.")
        else:
            st.success("Deze positie voldoet aan de norm (> 60).")
